    ...
    for (let i = 0; i < newItems.length; i++) {
      ...
      while (!success) {
        ...
        try {
          ...
          setItems(prev => prev.map(it => it.id === item.id ? { ...it, status: 'completed', audioUrl, isWaitingLimit: false, errorMsg: undefined } : it));
          success = true; 
        } catch (err: any) {
          const msg = err.message || "";
          
          // Jika terkena filter keamanan, kita terpaksa lewati karena ganti key pun tidak akan menolong
          if (msg.includes("Safety")) {
            setItems(prev => prev.map(it => it.id === item.id ? { ...it, status: 'error', errorMsg: "DIBLOKIR FILTER KEAMANAN (TIDAK BISA RETRY)" } : it));
            await new Promise(r => setTimeout(r, 2000));
            break; 
          }

          // Untuk SEMUA error lain (429 Limit, 500 Server, Connection Reset, dsb),
          // kita anggap ini masalah Key atau koneksi. Tahan di grup ini.
          setItems(prev => prev.map(it => it.id === item.id ? { ...it, isWaitingLimit: true, errorMsg: `GAGAL/LIMIT! GANTI API KEY (NYANGKUT DI GRUP ${item.groupIndex})` } : it));
          
          // Countdown rotasi 3 detik (memberi waktu user untuk klik Header)
          for (let t = 3; t > 0; t--) {
            setLimitWaitTime(t);
            await new Promise(r => setTimeout(r, 1000));
          }
          // Loop 'while' akan mengulang kembali ke 'generateSpeech' dengan API_KEY yang (diharapkan) sudah diganti user
        }
      }
